...
public class BackOficeController implements Initializable {
     ...
     ObservableList<Candidat> candidatList = FXCollections.observableArrayList();
      ObservableList<Candidat> candidaEntretienList = FXCollections.observableArrayList();
    ...
    public void initialize(URL url, ResourceBundle rb) {
     ...
     private void initCol() {
         ...
         statut.setCellValueFactory(new Callback<TableColumn.CellDataFeatures<Candidat, String>, ObservableValue<String>>() {
            ...
            public ObservableValue<String> call(TableColumn.CellDataFeatures<Candidat, String> param) {
               String result = param.getValue().getEntretient()==null?"":param.getValue().getEntretient().getStatutEnt().toString();
     ...
     public void loadData() {
        ...
        try {
            ...
            candidatList = FXCollections.observableArrayList(candidats);
            candidaEntretienList = FXCollections.observableArrayList(candidats.stream().filter(candidat->candidat.getEntretient()!=null).collect(Collectors.toList()));
            System.out.println(candidatList);
        } catch (SQLException ex) {
            
        }
        
        tableViewEntretient.setItems(candidaEntretienList);
    ...
    private void formation(ActionEvent event) {
     ...
     private void closeStage() {
        ((Stage) modifier.getScene().getWindow()).hide();
    }

  @FXML
    private void logout(MouseEvent event) {
        Rooting.navigate("Connexion", "login");
         affectationDate.getScene().getWindow().hide();
    }
